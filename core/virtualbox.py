...
class VirtualMachine:
    def __init__(self, vm_id = None):
        ...
        if vm_id is not None:
            self.name = OpenMAConfig().get_vm_name(vm_id)
            self.username = OpenMAConfig().get_vm_username(vm_id)
            self.password = OpenMAConfig().get_vm_password(vm_id)
    ...
    def check(self):
        ...
        if not re.match(VBOX_VERSION, self.vbox.version):
            log("[Virtual Machine] [Check] Your VirtualBox version" \
                " \"%s\" is not supported. You should upgrade to 5.x!"
    ...
    def execute(self, execName, args = None, timeout = None):
        ...
        if self.mach:
            ...
            if not timeout:
                watchdog = int(OpenMAConfig().get_analysis_watchdog_timeout())
